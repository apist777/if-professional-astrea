...
- **smoke-test.sh（CI, Clean環境）**: 初回PushでPart K（Professional Archiveの順序チェック）がFAILした。原因は`visible_content_only()`が`<script>`ブロックのみ除去しており、Archiveに新規追加した`core/read-more`が付与する隠しAccessible Name（`<span class="screen-reader-text">: {タイトル}</span>`）を除去していなかったため、氏名が可視H2タイトルと隠しSpanの2箇所でカウントされ二重化したこと。015C Part Yと同種の「新しい正当なMarkupに対するTest側のBlunt Checkの誤検知」であり、製品コードのAccessibility実装は意図通り。`visible_content_only()`に`.screen-reader-text`のStripを追加し、実機HTMLで氏名が1回のみカウントされることを確認した上で再Push。2回目のCIでは別のPart BB（Core非活性時にASTREA SEO出力が残っていないことの確認）がFAILした。原因は015Dでtheme.jsonへ追加したBreadcrumb用CSS（`.wp-block-astrea-breadcrumb`セレクタ）がTheme側の常時出力（Core状態に無関係）であるにもかかわらず、Testが`grep -qF 'wp-block-astrea-breadcrumb'`という素朴な部分一致でCheckしていたため、実際のBreadcrumb要素ではなくCSSセレクタ文字列を誤検知していたこと。これもPart Y/Kと同種のTest側Blunt Check誤検知。`class="wp-block-astrea-breadcrumb"`という実際のHTML属性値に一致条件を絞る修正をAU/BBの2箇所に適用し、実機HTMLで(1)実際の要素がある場合は1件、(2)CSSのみでHOMEのように要素が無い場合は0件、を正しく判別できることを確認した上で再々Push。3回目のCIではPart CO（Professional Singleで空Fieldが空見出しを残さないことを確認する、Construction 011由来の既存Check）がFAILした。原因は同じパターンで、015Dでtheme.jsonへ追加した`.wp-block-astrea-professional-field`のCSSが常時出力されるためだった。該当1項目のみ`class="..."`属性一致に絞る修正を行い、他に新規追加したCSSクラス（`astrea-archive-empty`/`astrea-archive-header`/`astrea-archive-listing`/`wp-block-astrea-closing-cta`/`astrea-professional-single-header`/`astrea-voice-quote`等）についても同種のBlunt Check箇所が他に存在しないことをsmoke-test.sh全体で確認した上で再々々Push。4回目のCIではPart CR（VOICE Archive項目見出しがH1からのLevel飛び無くH2で出力されることを確認する既存Check）がFAILした。今回は前3件と異なりTest側の誤検知ではなく、`archive-astrea_voice.html`のpost-titleブロックにAttribution装飾用の独自`className`を付与していたことで、既存Checkが期待する`<h2 class="wp-block-post-title">`という最小限のClass属性と一致しなくなっていたのが原因だった。Testではなく製品側のTemplateを修正: 独自`className`を外し、CSSセレクタを`.wp-block-astrea-voice-item .wp-block-post-title`という既存構造に依存するDescendant Selectorへ変更することで、Markup自体は既存Contract（`core/post-title`が余計なClassを持たないH2として出力される）を保ったまま視覚Styleのみ維持した。実機で見た目に変化が無いことを再確認した上で4回目のPush、CI Greenを確認した。
