...
4. **HeaderのNavigationブロックが、WordPress標準機構により実際にNavigationを自動作成する（GitHub Actions CIの実機実行で発見）。** Header/Footerに`ref`未指定の`core/navigation`ブロックを配置すると、WordPress core自身（`block_core_navigation_get_fallback_blocks()` / `WP_Navigation_Fallback`、WP 6.3以降の標準機構）が、そのブロックが最初にフロントエンドで描画された時点で、Page Listフォールバックを内容とする`wp_navigation`投稿（`post_status: publish`）を**自動的に作成・保存する**ことを実機検証で確認した（PHPUnitでは`render_block()`によるフロントエンド描画を経由しないため、この副作用は再現されない——実HTTP検証でのみ発見できた典型例）。これはASTREA固有の実装ではなく、`core/navigation`ブロックを使うあらゆるBlock Themeで共通の標準挙動である。結果として、Construction Order 007のSetup機能（Decision 016「既存Navigationが無い場合のみ生成提案」）は、Header/Footerが表示された時点でほぼ即座に「Navigation済み」と判定されるようになる。これはDecision 016・028のいずれとも矛盾しない（「既存のNavigationを上書き・重複生成しない」という核心要件は引き続き正しく守られる）が、実運用では「生成する」ボタンが表示される場面が想定より少なくなるという実務上の含意がある。仕様変更・新規Decisionは不要と判断し、`tools/ci/smoke-test.sh`のBIステップ（Construction 007由来）は、このステップの直前で既存の`wp_navigation`投稿を明示的に削除し、意図した「0件→ボタン提供→生成」の検証対象状態を再現するよう修正した。
5. **Query Loopベースの新規セクション（取扱業務Teaser）は仕様上の自己非表示ができない。** `home-services-teaser.php`はDecision 028の「HOME Teaser＝見出し含め完全非表示」を技術的に満たせない（Query Loopは`core/query-no-results`による代替コンテンツ表示はできても、見出しを含むセクション全体の消去はできない）。今回は明示承認された新規Dynamic Blockが`astrea/faq-list`のみであったため、取扱業務Teaserについては新規Blockを追加せず、Archive専用ページと同じ「前向きなメッセージ表示」規則を代わりに適用した。取扱業務専用の自己非表示Dynamic Block（`astrea/service-list`相当）を追加するかどうかは、次工程での確認事項として残す。
